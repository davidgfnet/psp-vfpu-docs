...
int run_vsgn_s(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[0] = rs[0] < 0 ? -1.0f : rs[0] > 0 ? 1.0f : 0.0f;
      ;
      float *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc0000e4" ::: "memory");
      asm volatile(".word 0xd04a0010" ::: "memory");
...
int run_vsgn_p(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[0] = rs[0] < 0 ? -1.0f : rs[0] > 0 ? 1.0f : 0.0f;
      rd[1] = rs[1] < 0 ? -1.0f : rs[1] > 0 ? 1.0f : 0.0f;
      ;
      float *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc0000e1" ::: "memory");
      asm volatile(".word 0xd04a0090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
...
int run_vsgn_t(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[0] = rs[0] < 0 ? -1.0f : rs[0] > 0 ? 1.0f : 0.0f;
      rd[1] = rs[1] < 0 ? -1.0f : rs[1] > 0 ? 1.0f : 0.0f;
      rd[2] = rs[2] < 0 ? -1.0f : rs[2] > 0 ? 1.0f : 0.0f;
      ;
      float *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc0000c9" ::: "memory");
      asm volatile(".word 0xd04a8010" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
...
int run_vsgn_q(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[1] = rs[1] < 0 ? -1.0f : rs[1] > 0 ? 1.0f : 0.0f;
      rd[2] = rs[2] < 0 ? -1.0f : rs[2] > 0 ? 1.0f : 0.0f;
      rd[3] = rs[3] < 0 ? -1.0f : rs[3] > 0 ? 1.0f : 0.0f;
      ;
      float *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc000039" ::: "memory");
      asm volatile(".word 0xd04a8090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S402, 2*4(%0)" ::"r"(&rd_out[0]));
