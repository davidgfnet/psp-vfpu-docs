...
int run_vscmp_s(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 16384; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[0] = rs[0] < rt[0] ? -1.0f : rs[0] > rt[0] ? 1.0f : 0.0f;
      ;
      float *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc0000e4" ::: "memory");
      asm volatile(".word 0x6e840010" ::: "memory");
...
int run_vscmp_p(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 16384; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[0] = rs[0] < rt[0] ? -1.0f : rs[0] > rt[0] ? 1.0f : 0.0f;
      rd[1] = rs[1] < rt[1] ? -1.0f : rs[1] > rt[1] ? 1.0f : 0.0f;
      ;
      float *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc0000e1" ::: "memory");
      asm volatile(".word 0x6e840090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
...
int run_vscmp_t(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 16384; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[0] = rs[0] < rt[0] ? -1.0f : rs[0] > rt[0] ? 1.0f : 0.0f;
      rd[1] = rs[1] < rt[1] ? -1.0f : rs[1] > rt[1] ? 1.0f : 0.0f;
      rd[2] = rs[2] < rt[2] ? -1.0f : rs[2] > rt[2] ? 1.0f : 0.0f;
      ;
      float *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc0000c9" ::: "memory");
      asm volatile(".word 0x6e848010" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
...
int run_vscmp_q(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 16384; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[1] = rs[1] < rt[1] ? -1.0f : rs[1] > rt[1] ? 1.0f : 0.0f;
      rd[2] = rs[2] < rt[2] ? -1.0f : rs[2] > rt[2] ? 1.0f : 0.0f;
      rd[3] = rs[3] < rt[3] ? -1.0f : rs[3] > rt[3] ? 1.0f : 0.0f;
      ;
      float *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc000039" ::: "memory");
      asm volatile(".word 0x6e848090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S402, 2*4(%0)" ::"r"(&rd_out[0]));
