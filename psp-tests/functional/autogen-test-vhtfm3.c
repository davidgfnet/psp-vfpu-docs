...
int run_vhtfm3_t(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 16384; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[0] = rs[3 * 0] * rt[0] + rs[3 * 0 + 1] * rt[1] + rs[3 * 0 + 2];
      rd[1] = rs[3 * 1] * rt[0] + rs[3 * 1 + 1] * rt[1] + rs[3 * 1 + 2];
      rd[2] = rs[3 * 2] * rt[0] + rs[3 * 2 + 1] * rt[1] + rs[3 * 2 + 2];
      ;
      float *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdd0000c9" ::: "memory");
      asm volatile(".word 0xf1040090" ::: "memory");
