...
int run_vavg_p(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0470090" ::: "memory");
...
int run_vavg_t(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0478010" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      int mm = (fpdifn(&rd_final[0], &rd_out[0], 1));
      if (mm) {
        struct test_error_info *rep = &pfxinfo->rs_swz_err;
...
int run_vavg_q(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0478090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      int mm = (fpdifn(&rd_final[0], &rd_out[0], 1));
      if (mm) {
        struct test_error_info *rep = &pfxinfo->rs_swz_err;
