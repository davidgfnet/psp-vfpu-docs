...
int run_vocp_s(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0440010" ::: "memory");
...
int run_vocp_p(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0440090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
...
int run_vocp_t(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0448010" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
...
int run_vocp_q(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0448090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S402, 2*4(%0)" ::"r"(&rd_out[0]));
