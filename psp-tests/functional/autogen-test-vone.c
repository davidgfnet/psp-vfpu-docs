...
int run_vone_p(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 1; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xde000f00" ::: "memory");
      asm volatile(".word 0xd0070090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
      int mm = (fpdifn(&rd_final[0], &rd_out[0], 2));
      if (mm) {
        struct test_error_info *rep = &pfxinfo->rd_msk_err;
        static const struct test_decl_info iinfo = {.inst = "vone.p",
                                                    .instw = 0xd0070090,
                                                    .pfxw = 0xde000f00,
                                                    .rdoutsz = 2,
                                                    .inputs = {}};
...
int run_vone_t(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 1; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xde000f00" ::: "memory");
      asm volatile(".word 0xd0078010" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
      ...
      if (mm) {
        struct test_error_info *rep = &pfxinfo->rd_msk_err;
        static const struct test_decl_info iinfo = {.inst = "vone.t",
                                                    .instw = 0xd0078010,
                                                    .pfxw = 0xde000f00,
                                                    .rdoutsz = 3,
                                                    .inputs = {}};
...
int run_vone_q(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 1; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xde000f00" ::: "memory");
      asm volatile(".word 0xd0078090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S402, 2*4(%0)" ::"r"(&rd_out[0]));
      ...
      if (mm) {
        struct test_error_info *rep = &pfxinfo->rd_msk_err;
        static const struct test_decl_info iinfo = {.inst = "vone.q",
                                                    .instw = 0xd0078090,
                                                    .pfxw = 0xde000f00,
                                                    .rdoutsz = 4,
                                                    .inputs = {}};
