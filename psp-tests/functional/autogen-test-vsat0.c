...
int run_vsat0_s(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0040010" ::: "memory");
...
int run_vsat0_p(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0040090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
...
int run_vsat0_t(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0048010" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
...
int run_vsat0_q(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0048090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S402, 2*4(%0)" ::"r"(&rd_out[0]));
