...
int run_vi2us_p(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[0] = (rs[0 * 2 + 0] & 0x80000000 ? 0 : ((rs[0 * 2 + 0] >> 15) << 0)) |
              (rs[0 * 2 + 1] & 0x80000000 ? 0 : ((rs[0 * 2 + 1] >> 15) << 16));
      ;
      unsigned int *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc0000e1" ::: "memory");
      asm volatile(".word 0xd03e0090" ::: "memory");
...
int run_vi2us_q(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      rd[0] = (rs[0 * 2 + 0] & 0x80000000 ? 0 : ((rs[0 * 2 + 0] >> 15) << 0)) |
              (rs[0 * 2 + 1] & 0x80000000 ? 0 : ((rs[0 * 2 + 1] >> 15) << 16));
      rd[1] = (rs[1 * 2 + 0] & 0x80000000 ? 0 : ((rs[1 * 2 + 0] >> 15) << 0)) |
              (rs[1 * 2 + 1] & 0x80000000 ? 0 : ((rs[1 * 2 + 1] >> 15) << 16));
      ;
      unsigned int *rd_final = &rd[0]; // No dst prefix
      asm volatile(".word 0xdc000039" ::: "memory");
      asm volatile(".word 0xd03e8090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
