...
int run_vneg_s(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0020010" ::: "memory");
...
int run_vneg_p(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0020090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
...
int run_vneg_t(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0028010" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
...
int run_vneg_q(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 128; i++) {
    ...
    pfxinfo->rd_tested = 1;
    {
      ...
      asm volatile(".word 0xd0028090" ::: "memory");
      asm volatile("sv.s S400, 0*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S401, 1*4(%0)" ::"r"(&rd_out[0]));
      asm volatile("sv.s S402, 2*4(%0)" ::"r"(&rd_out[0]));
