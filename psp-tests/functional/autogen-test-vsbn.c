...
int run_vsbn_s(struct test_error_info *errlist,
  ...
  for (unsigned i = 0; i < 16384; i++) {
    ...
    gen_inputs_u32(i >> 7, &rt_ref[0], 1);
    pfxinfo->inst_name = "vsbn.s";
    pfxinfo->rs_tested = 1;
    pfxinfo->rt_tested = 1;
    pfxinfo->rd_tested = 1;
    {
      ...
      unsigned int *rt = &rt_ref[0]; // No src prefix
